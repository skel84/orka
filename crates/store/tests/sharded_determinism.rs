...
async fn run_sequence(seq: &[Delta]) -> Vec<(String, String, u8)> {
    ...
    let mut canon: Vec<(String, String, u8)> = snap
        .items
        .iter()
        .map(|o| {
            (
                o.namespace.clone().unwrap_or_else(|| "".into()),
                o.name.clone(),
                o.uid[0],
            )
        })
...
async fn deterministic_across_runs() {
    ...
    let seq = vec![
        ...
        Delta {
            ...
            raw: obj("a", Some("ns1"), "2020-01-01T00:00:00Z"),
        },
        Delta {
            uid: uid(2),
            kind: DeltaKind::Applied,
            raw: obj("b", Some("ns2"), "2020-01-01T00:00:01Z"),
        },
        Delta {
            uid: uid(3),
            kind: DeltaKind::Applied,
            raw: obj("c", Some("ns3"), "2020-01-01T00:00:02Z"),
        },
        // out-of-order update and duplicate events
        Delta {
            uid: uid(2),
            kind: DeltaKind::Applied,
            raw: obj("bb", Some("ns2"), "2020-01-01T00:00:01Z"),
        },
        Delta {
            uid: uid(2),
            kind: DeltaKind::Applied,
        ...
        Delta {
            uid: uid(4),
            kind: DeltaKind::Applied,
            raw: obj("d", Some("prod"), "2020-01-01T00:00:03Z"),
        },
    ];
    ...
    assert_eq!(
        c1, c2,
        "canonical snapshot view must be deterministic across runs"
    );
