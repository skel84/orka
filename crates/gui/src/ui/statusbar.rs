        ...
        .show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if let Some(s) = &app.stats.data {
                    ui.label(format!("pipe: {}", s.shards));
                }
                if let Some(epoch) = app.results.epoch {
                    ui.separator();
                    ui.label(format!("epoch: {}", epoch));
                }
                // Memory cap banners (static caps)
                if let Some(s) = &app.stats.data {
                    if let Some(mb) = s.max_rss_mb {
                        ui.separator();
                        ui.colored_label(ui.visuals().warn_fg_color, format!("MaxRSS {} MB", mb));
                    }
                    if let Some(bytes) = s.max_index_bytes {
                        ...
                        if let Some(cur) = app.stats.index_bytes {
                            let pct = if bytes > 0 {
                                (cur as f32) / (bytes as f32)
                            } else {
                                0.0
                            };
                            let col = if pct >= app.stats.err_pct {
                                ui.visuals().error_fg_color
                            } else if pct >= app.stats.warn_pct {
                                ui.visuals().warn_fg_color
                            } else {
                ...
                if app.logs.dropped > 0 || (app.logs.running && app.logs.recv > 0) {
                    ...
                    if app.logs.recv > 0 {
                        ui.separator();
                        ui.label(format!("logs recv: {}", app.logs.recv));
                    }
                    if app.logs.dropped > 0 {
                        ui.separator();
                        ui.colored_label(
                            ui.visuals().error_fg_color,
                            format!("dropped: {}", app.logs.dropped),
                        );
                    }
                }
