...
enum AnsiColor { Default, Index(u8) }

#[derive(Clone, Debug)]
struct Cell { ch: char, fg: AnsiColor }

impl Default for Cell { fn default() -> Self { Self { ch: ' ', fg: AnsiColor::Default } } }
...
impl AnsiTerm {
    ...
    fn resize(&mut self, cols: u16, rows: u16) {
        ...
        self.grid.clear();
        self.grid.resize((cols as usize) * (rows as usize), Cell::default());
        self.cur_x = 0; self.cur_y = 0;
    }
    fn idx(&self, x: u16, y: u16) -> usize { (y as usize) * (self.cols as usize) + (x as usize) }
    fn put_char(&mut self, c: char) {
        if c == '\n' { self.newline(); return; }
        if c == '\r' { self.cur_x = 0; return; }
        if c == '\t' { let tab = 4; let nx = ((self.cur_x as usize / tab) + 1) * tab; self.cur_x = (nx.min(self.cols as usize - 1)) as u16; return; }
        if self.cur_x >= self.cols { self.newline(); }
        if self.cur_y >= self.rows { self.scroll_up(1); self.cur_y = self.rows - 1; }
        let idx = self.idx(self.cur_x, self.cur_y);
        if idx < self.grid.len() { self.grid[idx] = Cell { ch: c, fg: self.fg }; }
        if self.cur_x + 1 >= self.cols { self.newline(); } else { self.cur_x += 1; }
    }
    fn newline(&mut self) { self.cur_x = 0; if self.cur_y + 1 >= self.rows { self.scroll_up(1); } else { self.cur_y += 1; } }
    fn backspace(&mut self) { if self.cur_x > 0 { self.cur_x -= 1; let idx = self.idx(self.cur_x, self.cur_y); self.grid[idx] = Cell::default(); } }
    fn clear_screen(&mut self) { for cell in &mut self.grid { *cell = Cell::default(); } self.cur_x = 0; self.cur_y = 0; }
    fn clear_eol(&mut self) { let y = self.cur_y; for x in self.cur_x..self.cols { let idx = self.idx(x, y); self.grid[idx] = Cell::default(); } }
    fn move_to(&mut self, col1: u16, row1: u16) { self.cur_x = col1.saturating_sub(1).min(self.cols.saturating_sub(1)); self.cur_y = row1.saturating_sub(1).min(self.rows.saturating_sub(1)); }
    ...
    fn scroll_up(&mut self, n: u16) {
        ...
        self.grid.extend(std::iter::repeat(Cell::default()).take(count));
    }
    fn set_sgr(&mut self, params: &[i64]) {
        if params.is_empty() { self.fg = AnsiColor::Default; return; }
        let mut it = params.iter();
        while let Some(p) = it.next() {
            match *p {
                0 => { self.fg = AnsiColor::Default; }
                30..=37 => { self.fg = AnsiColor::Index(((*p as i32) - 30) as u8); }
                90..=97 => { self.fg = AnsiColor::Index(((*p as i32) - 90 + 8) as u8); }
                39 => { self.fg = AnsiColor::Default; }
                38 => { // 256-color not implemented; skip params
...
impl vte::Perform for AnsiTerm {
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, _intermediates: &[u8], _ignore: bool, action: char) {
        let nums: Vec<i64> = params.iter().map(|p| if p.is_empty() { 0 } else { p[0] as i64 }).collect();
        match action {
            'H' | 'f' => {
                let row = if nums.len() >= 1 && nums[0] > 0 { nums[0] as u16 } else { 1 };
                let col = if nums.len() >= 2 && nums[1] > 0 { nums[1] as u16 } else { 1 };
                self.move_to(col, row);
            }
            'A' => { let n = if nums.is_empty() || nums[0] == 0 { 1 } else { nums[0] as i16 }; self.move_rel(0, -n); }
            'B' => { let n = if nums.is_empty() || nums[0] == 0 { 1 } else { nums[0] as i16 }; self.move_rel(0, n); }
            'C' => { let n = if nums.is_empty() || nums[0] == 0 { 1 } else { nums[0] as i16 }; self.move_rel(n, 0); }
            'D' => { let n = if nums.is_empty() || nums[0] == 0 { 1 } else { nums[0] as i16 }; self.move_rel(-n, 0); }
            'G' => { let col = if nums.is_empty() || nums[0] == 0 { 1 } else { nums[0] as u16 }; self.move_to(col, self.cur_y + 1); }
            'J' => { // Erase in display
                let mode = if nums.is_empty() { 0 } else { nums[0] };
                if mode == 2 { self.clear_screen(); } else if mode == 0 { self.clear_eol(); }
            }
            'K' => { self.clear_eol(); }
            'm' => { self.set_sgr(&nums); }
            _ => {}
...
impl UiTerminal {
    pub fn new() -> Self {
        Self { parser: vte::Parser::new(), term: AnsiTerm::new(80, 24), last_cols: None, last_rows: None }
    }

    /// Feed raw bytes into the terminal (ANSI sequences supported when feature is on).
    pub fn feed_bytes(&mut self, bytes: &[u8]) {
        for &b in bytes { self.parser.advance(&mut self.term, b); }
    }
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) -> (u16, u16, bool) {
        ...
        let char_w = ui.fonts(|f| f.glyph_width(&mono.resolve(ui.style()), 'W')).max(6.0);
        ...
        for row in 0..rows {
            ...
            for col in 0..cols {
                ...
                if idx >= self.term.grid.len() { break; }
                ...
                if color != run_color {
                    if !run.is_empty() { job.append(&run, 0.0, egui::TextFormat { color: run_color, ..egui::TextFormat::default() }); run.clear(); }
                    run_color = color;
                }
                run.push(c);
            }
            if !run.is_empty() { job.append(&run, 0.0, egui::TextFormat { color: run_color, ..egui::TextFormat::default() }); }
            child.add(egui::Label::new(job).truncate());
        }
        if resp.clicked() { ui.memory_mut(|m| m.request_focus(resp.id)); }
