...
pub(crate) fn ui_palette(app: &mut OrkaGuiApp, ctx: &egui::Context) {
    ...
    egui::Window::new("cmd_k_palette")
        ...
        .show(ctx, |ui| {
            ...
            let resp = ui.add(te);
            if app.palette.need_focus { resp.request_focus(); app.palette.need_focus = false; }
            if resp.changed() { app.palette.changed_at = Some(std::time::Instant::now()); }
            if ui.input(|i| i.key_pressed(egui::Key::Escape)) { app.palette.open = false; }
            // Debounce build
            if let Some(t0) = app.palette.changed_at { if t0.elapsed().as_millis() as u64 >= app.palette.debounce_ms { app.rebuild_palette_results(); app.palette.changed_at = None; } }
            ui.separator();
            // Mode toggle: Cached vs Global (prime watchers)
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Mode:").weak());
                let cached = !app.palette.mode_global;
                if ui.selectable_label(cached, "Cached").clicked() { app.palette.mode_global = false; }
                if ui.selectable_label(!cached, "Global").clicked() {
                    if !app.palette.mode_global { app.palette.mode_global = true; app.start_palette_global_prime(); }
                }
            });
            ui.add_space(4.0);
            // Keyboard selection
            ...
            if ui.input(|i| i.key_pressed(egui::Key::ArrowDown)) {
                ...
                if len > 0 { let cur = app.palette.sel.unwrap_or(usize::MAX); app.palette.sel = Some(if cur == usize::MAX { 0 } else { (cur + 1) % len }); }
            }
            if ui.input(|i| i.key_pressed(egui::Key::ArrowUp)) {
                let len = app.palette.results.len();
                if len > 0 { let cur = app.palette.sel.unwrap_or(0); app.palette.sel = Some(if cur == 0 { len - 1 } else { cur - 1 }); }
            }
            let enter = ui.input(|i| i.key_pressed(egui::Key::Enter));
            let esc = ui.input(|i| i.key_pressed(egui::Key::Escape));
            if esc { app.palette.open = false; }
            ...
            if enter {
                if let Some(sel) = app.palette.sel.and_then(|i| app.palette.results.get(i).cloned()) { chosen = Some(sel); }
            }
            if let Some(item) = chosen.take() { app.open_palette_item(item); app.palette.open = false; }
        });
...
impl OrkaGuiApp {
    pub(crate) fn rebuild_palette_results(&mut self) {
        ...
        for (gvk_key, it) in all.into_iter() {
            ...
            let hay = self.results.filter_cache.get(&it.uid).cloned().unwrap_or_else(|| self.build_filter_haystack(&it));
        ...
        self.palette.sel = if self.palette.results.is_empty() { None } else { Some(0) };
    ...
    pub(crate) fn open_palette_item(&mut self, item: PaletteItem) {
        ...
        let rk = self.discovery.kinds.iter().find(|k| gvk_label(k) == item.gvk_key).cloned().unwrap_or_else(|| parse_gvk_key_to_kind(&item.gvk_key));
    ...
    pub(crate) fn start_palette_global_prime(&mut self) {
        if self.palette.prime_task.is_some() { return; }
        let api = self.api.clone();
        let kinds_opt = if !self.discovery.kinds.is_empty() { Some(self.discovery.kinds.clone()) } else { None };
