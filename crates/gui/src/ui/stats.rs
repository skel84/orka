        ...
        .show(ctx, |ui| {
            ui.vertical(|ui| {
                ...
                if let Some(s) = &app.stats.data {
                    ...
                    if let Some(v) = s.max_annos_per_obj { grid_kv(ui, "Max annotations per object", &v.to_string()); }
                    if let Some(v) = s.max_postings_per_key { grid_kv(ui, "Max postings per key", &v.to_string()); }
                    if let Some(v) = s.max_rss_mb { grid_threshold(ui, "Max RSS (MB)", v as u64, None, app.stats.warn_pct, app.stats.err_pct); }
                    if let Some(v) = s.max_index_bytes { grid_threshold(ui, "Index bytes", v as u64, app.stats.index_bytes, app.stats.warn_pct, app.stats.err_pct); }
                ...
                let dropped_color = if dropped > 0 { ui.visuals().error_fg_color } else { ui.visuals().text_color() };
...
fn grid_threshold(ui: &mut egui::Ui, k: &str, max: u64, cur_opt: Option<u64>, warn: f32, err: f32) {
    match cur_opt {
        Some(cur) => {
            let pct = if max > 0 { (cur as f32) / (max as f32) } else { 0.0 };
            let color = if pct >= err { ui.visuals().error_fg_color } else if pct >= warn { ui.visuals().warn_fg_color } else { ui.visuals().text_color() };
