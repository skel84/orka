...
use crate::watch::{watch_hub_prime, watch_hub_subscribe};
        ...
        match rx.try_recv() {
            Ok(Ok(mut v)) => {
                ...
                if !app.watch.prewarm_started {
                    ...
                    let prewarm_all = std::env::var("ORKA_PREWARM_ALL_BUILTINS")
                        .ok()
                    ...
                    if prewarm_all {
                        ...
                        let allowed: std::collections::HashSet<String> = groups_env
                            .split(',')
                            .map(|s| s.trim().to_string())
                            .collect();
                        let conc: usize = std::env::var("ORKA_PREWARM_CONC")
                            .ok()
                            .and_then(|s| s.parse().ok())
                            .unwrap_or(4);
                        let sem = std::sync::Arc::new(Semaphore::new(conc.max(1)));
                        for k in app.discovery.kinds.iter() {
                            let group_key = if k.group.is_empty() {
                                "core".to_string()
                            } else {
                                k.group.clone()
                            };
                            if !allowed.contains(&group_key) {
                                continue;
                            }
