...
pub(crate) fn handle_global_shortcuts(app: &mut OrkaGuiApp, ctx: &egui::Context) {
    ...
    if ctx.input(|i| i.key_pressed(egui::Key::L)) {
        ...
        if can_logs {
            if app.logs.running {
                app.stop_logs_task();
                app.toast("logs: stopped", ToastKind::Info);
            } else {
                app.start_logs_task();
            }
        } else {
    ...
    if ctx.input(|i| i.key_pressed(egui::Key::E)) {
        let can_exec = app.selected_is_pod()
            && app
                ...
                .caps
                .as_ref()
                .map(|c| c.pods_exec_create)
                .unwrap_or(false);
        if can_exec {
            app.details.active_tab = crate::model::DetailsPaneTab::Exec;
            app.start_exec_task();
        } else {
            app.toast("exec: unavailable (select Pod or RBAC)", ToastKind::Warn);
        }
    }
    ...
    if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
        ...
        if app.search.task.is_some() {
            if let Some(stop) = app.search.stop.take() {
                let _ = stop.send(());
            }
            app.search.task = None;
            app.toast("search: canceled", ToastKind::Info);
        }
        // Cancel details fetch
        if let Some(stop) = app.details.stop.take() {
            let _ = stop.send(());
            app.toast("details: canceled", ToastKind::Info);
        }
        ...
        if app.exec.running {
            app.stop_exec_task();
            app.toast("exec: stopped", ToastKind::Info);
        }
