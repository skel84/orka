...
pub(crate) fn process_updates(app: &mut OrkaGuiApp, ctx: &egui::Context) {
    ...
    if let Some(rx) = &app.watch.updates_rx {
        while processed < 256 {
            match rx.try_recv() {
                Ok(UiUpdate::Snapshot(items)) => {
                    ...
                    if app.results.rows.is_empty() {
                        ...
                        for (i, it) in app.results.rows.iter().enumerate() {
                            ...
                            app.results.filter_cache.insert(it.uid, app.build_filter_haystack(it));
                        }
                        info!(items = count, total = app.results.rows.len(), "ui: snapshot applied (initial)");
                        ...
                        for it in items.into_iter() {
                            if !app.results.index.contains_key(&it.uid) {
                                ...
                                app.results.filter_cache.insert(it.uid, app.build_filter_haystack(&it));
                                app.results.display_cache.insert(it.uid, app.build_display_row(&it));
                                app.results.rows.push(it);
                            }
                        }
                        info!(added = app.results.rows.len() - pre_total, total = app.results.rows.len(), "ui: snapshot merged (incremental)");
                    }
                    app.last_error = None;
                        ...
                        LiteEvent::Applied(lo) => {
                            ...
                            if let Some(idx) = app.results.index.get(&uid).copied() {
                                if idx < app.results.rows.len() {
                                    app.results.filter_cache.insert(lo.uid, app.build_filter_haystack(&lo));
                                    app.results.display_cache.insert(lo.uid, app.build_display_row(&lo));
                                    app.results.rows[idx] = lo;
                                } else {
                                    let exists_as_last = if !app.results.rows.is_empty() { let last_idx = app.results.rows.len() - 1; app.results.rows.get(last_idx).map(|r| r.uid == uid).unwrap_or(false) } else { false };
                                    if exists_as_last {
                                        let last_idx = app.results.rows.len() - 1;
                                        app.results.rows[last_idx] = lo.clone();
                                        app.results.index.insert(uid, last_idx);
                                        app.results.filter_cache.insert(uid, app.build_filter_haystack(&lo));
                                        app.results.display_cache.insert(uid, app.build_display_row(&lo));
                        ...
                        LiteEvent::Deleted(lo) => {
                            ...
                            if let Some(idx) = app.results.index.remove(&uid) {
                                if idx < app.results.rows.len() {
                                    ...
                                    if let Some(swapped) = app.results.rows.get(idx) { app.results.index.insert(swapped.uid, idx); }
                ...
                Ok(UiUpdate::DetailError(err)) => { app.last_error = Some(err); processed += 1; }
                Ok(UiUpdate::Namespaces(v)) => { app.namespaces = v; processed += 1; }
                ...
                Ok(UiUpdate::Epoch(e)) => { app.results.epoch = Some(e); processed += 1; }
                ...
                Ok(UiUpdate::EditStatus(s)) => { app.edit.status = s; processed += 1; }
                Ok(UiUpdate::EditDryRunDone { summary }) => { app.edit.running = false; app.edit.status = format!("dry-run: {}", summary); pending_toasts.push((format!("dry-run: {}", summary), ToastKind::Info)); processed += 1; }
                Ok(UiUpdate::EditDiffDone { live, last }) => {
                    app.edit.running = false;
                    app.edit.status = match last { Some(s) => format!("diff live: {}  •  vs last-applied: {}", live, s), None => format!("diff live: {}", live) };
                ...
                Err(mpsc::TryRecvError::Disconnected) => { app.watch.updates_rx = None; break; }
            }
        }
        ...
        if !reattach_requests.is_empty() {
            for (id, uid) in reattach_requests.drain(..) {
                ...
                if let Some(i) = app.results.index.get(&uid).copied() {
                    if let Some(row) = app.results.rows.get(i).cloned() { app.select_row(row); }
                } else {
        ...
        if processed > 0 {
            ...
            if app.ui_debounce.pending_since.is_none() { app.ui_debounce.pending_since = Some(std::time::Instant::now()); }
            let elapsed_ms = app.ui_debounce.pending_since.map(|t| t.elapsed().as_millis() as u64).unwrap_or(0);
            let should_flush = saw_batch || app.ui_debounce.pending_count >= 256 || elapsed_ms >= app.ui_debounce.ms;
            if should_flush {
                let processed_now = app.ui_debounce.pending_count as u64;
                info!(processed = processed_now, total = app.results.rows.len(), "ui: flushed updates");
    ...
    for (text, kind) in pending_toasts.drain(..) { app.toast(text, kind); }
}
