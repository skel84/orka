...
pub(crate) fn ui_actions_bar(app: &mut OrkaGuiApp, ui: &mut egui::Ui) {
    ...
    ui.horizontal(|ui| {
        ...
        if logs_ok {
            ...
            ui.add_enabled(false, egui::Button::new("Logs"))
                .on_hover_text(if !pod_sel {
                    "Select a Pod"
                } else {
                    "RBAC: pod/log get denied"
                });
        }
        ...
        if exec_ok {
            if ui.button("Exec…").clicked() {
                info!("ui: exec click");
                app.details.active_tab = crate::model::DetailsPaneTab::Exec;
                app.start_exec_task();
            }
            if ui
                .button("Open in Terminal")
                .on_hover_text("Launch external terminal (Alacritty preferred) with kubectl exec")
                .clicked()
            {
            ...
            ui.add_enabled(false, egui::Button::new("Exec…"))
                .on_hover_text("RBAC: pods/exec or Pod selection missing");
        }
        ...
        let pf_ok = pod_sel
            && caps
                .as_ref()
                .map(|c| c.pods_portforward_create)
                .unwrap_or(false);
        ui.label("PF:");
        ...
        if !app.ops.pf_candidates.is_empty() {
            ...
            let mut sel = app
                ...
                .pf_selected_idx
                .unwrap_or(0)
                .min(app.ops.pf_candidates.len().saturating_sub(1));
        ...
        if local != app.ops.pf_local {
            app.ops.pf_local = local;
        }
        if remote != app.ops.pf_remote {
            app.ops.pf_remote = remote;
        }
        if !app.ops.pf_running {
            if pf_ok {
                if ui.button("Start").clicked() {
                    info!("ui: pf start click");
                    app.start_port_forward_task();
                }
            } else {
                ui.add_enabled(false, egui::Button::new("Start"))
                    .on_hover_text("RBAC: pods/portforward or Pod selection missing");
            }
        } else {
            let stop_pf = ui.button("Stop");
            if stop_pf.clicked() {
                info!("ui: pf stop click");
        ...
        if scale_ok {
            if ui
                .button("Apply")
                .on_hover_text("Set replicas to this value")
                .clicked()
            {
                info!(replicas = r, "ui: scale apply click");
                app.ops.scale_replicas = r;
                app.start_scale_task();
            }
        } else {
            ui.add_enabled(false, egui::Button::new("Apply"))
                .on_hover_text(if has_caps {
                    "Not scalable"
                } else {
                    "Probing caps…"
                });
        ...
        if rr_ok && rr_btn.clicked() {
            app.start_rollout_restart_task();
        }
        if !rr_ok {
            rr_btn.on_hover_text(if has_caps {
        ...
        if app.selected_is_node() {
            ...
            if can_patch_nodes {
                if ui.button("Cordon").clicked() {
                    info!("ui: cordon click");
                    app.start_cordon_task(true);
                }
                if ui.button("Uncordon").clicked() {
                    info!("ui: uncordon click");
                    app.start_cordon_task(false);
                }
            } else {
                ui.add_enabled(false, egui::Button::new("Cordon"))
                    .on_hover_text("RBAC: nodes patch denied");
                ui.add_enabled(false, egui::Button::new("Uncordon"))
                    .on_hover_text("RBAC: nodes patch denied");
            }
            ...
            if can_drain {
                if ui.button("Drain…").clicked() {
                    info!("ui: drain confirm open");
                    if let Some(node) = app.current_node_selection() {
                        app.ops.confirm_drain = Some(node);
                    }
                }
            } else {
                ui.add_enabled(false, egui::Button::new("Drain…"))
                    .on_hover_text("RBAC: nodes patch or evictions denied");
            }
        }
        ...
        if ui.button(format!("PFs ({})", pf_count)).clicked() {
            info!("ui: pf panel open");
            app.ops.pf_panel_open = true;
        }
    });
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Cancel").clicked() {
                        app.ops.confirm_delete = None;
                    }
                    if ui.button("Delete").clicked() {
                        info!("ui: delete pod confirm");
                        app.start_delete_pod_task();
                        app.ops.confirm_delete = None;
                    }
                });
            });
    ...
    if let Some(node) = app.ops.confirm_drain.clone() {
        ...
        egui::Window::new("Confirm Drain Node")
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("Cancel").clicked() {
                        app.ops.confirm_drain = None;
                    }
                    if ui.button("Drain").clicked() {
                        info!("ui: drain confirm");
                        app.start_drain_task();
                        app.ops.confirm_drain = None;
                    }
                });
            });
    ...
    if app.ops.scale_prompt_open {
        ...
        egui::Window::new("Scale Workload")
            ...
            .show(ui.ctx(), |ui| {
                if !caps_ok {
                    ui.label("Not scalable for this kind");
                    return;
                }
                ui.label("Replicas:");
                let mut r = app.ops.scale_replicas.max(0);
                if ui
                    .add(egui::DragValue::new(&mut r).range(0..=10000))
                    .changed()
                {
                    app.ops.scale_replicas = r;
                }
                ui.horizontal(|ui| {
                    if ui.button("Cancel").clicked() {
                        app.ops.scale_prompt_open = false;
                    }
        ...
        egui::Window::new("Active Port-Forwards")
            ...
            .show(ui.ctx(), |ui| {
                if let Some(info) = app.ops.pf_info.clone() {
                    let line = format!(
                        "{}/{}  {} → {}",
                        info.namespace, info.pod, info.local, info.remote
                    );
                    ui.horizontal(|ui| {
                        ui.label(line);
                        if ui.button("Stop").clicked() {
                            stop_pf_clicked = true;
                        }
                    });
                } else {
                    ui.label("None");
