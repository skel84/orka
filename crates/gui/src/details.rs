...
use crate::model::{GraphNodeRole, GraphViewMode};
...
impl OrkaGuiApp {
    ...
    fn ui_edit(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            if ui
                ...
                .on_hover_text("Reset editor to current Details")
                .clicked()
            {
            ...
            if ui
                .button("Dry-run")
                .on_hover_text("Server-side dry-run; show diff summary")
                .clicked()
            {
            ...
            if ui
                .button("Diff")
                .on_hover_text("Compute diff vs live and last-applied")
                .clicked()
            {
            ...
            if ui
                .button("Apply")
                .on_hover_text("Server-side apply (SSA)")
                .clicked()
            {
                self.start_edit_apply_task();
            }
            if self.edit.running {
                ...
                if self.details.secret_entries.is_empty() {
                    ...
                    for entry in self.details.secret_entries.clone() {
                        ...
                        let display = if revealed {
                            if let Some(s) = &entry.decoded {
                                s.clone()
                            } else {
                                format!("(binary) base64={}", entry.b64)
                            }
                        } else {
                            "••••••".to_string()
                        };
            ...
            .show(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if out.response.changed() {
                        self.edit.dirty = self.edit.buffer != self.edit.original;
                    }
                    ...
                    if let Some(cr) = out.cursor_range {
                        ...
                        let line_idx = self.edit.buffer[..idx]
                            .chars()
                            .filter(|&c| c == '\n')
                            .count();
    ...
    fn ui_logs(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("logs_prefix_theme")
                .selected_text(match theme {
                    crate::model::PrefixTheme::Bright => "Bright",
                    crate::model::PrefixTheme::Basic => "Basic",
                    crate::model::PrefixTheme::Gray => "Gray",
                    crate::model::PrefixTheme::None => "None",
                })
                .show_ui(ui, |ui| {
            ...
            if theme != self.logs.prefix_theme {
                self.logs.prefix_theme = theme;
                if self.logs.running && matches!(self.logs.container.as_deref(), Some("(all)")) {
            ...
            if ui
                .add(egui::DragValue::new(&mut vis).range(100..=self.logs.ring_cap as i32))
                .on_hover_text("Max visible lines when following")
                .changed()
            {
            ...
            if ui
                .add(egui::DragValue::new(&mut since).range(0..=86_400))
            ...
            if ui
                .add(egui::DragValue::new(&mut tail).range(0..=10000))
            ...
            if self.logs.containers.is_empty() {
                ...
                egui::ComboBox::from_id_salt("logs_container_select")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        if ui
                            .selectable_value(&mut selected, "(all)".to_string(), "(all)")
                            .changed()
                        {
                        ...
                        for name in &self.logs.containers {
                            if ui
                                .selectable_value(&mut selected, name.clone(), name)
                                .changed()
                            {
                                changed_container = true;
                            }
                ...
                if selected != current {
                    ...
                    if self.logs.running && changed_container {
                        self.start_logs_task();
                    }
                }
            }
        ...
        ui.horizontal(|ui| {
            ...
            if self.logs.dropped > 0 {
                ...
                ui.colored_label(
                    ...
                    format!("dropped: {}", self.logs.dropped),
                );
            }
            ...
            if !self.logs.running {
                if ui
                    .button("Start")
                    .on_hover_text("Start streaming logs")
                    .clicked()
                {
                    self.start_logs_task();
                }
        ...
        let mut indices: Vec<usize> = if let Some((_text, re)) = &self.logs.grep_cache {
            self.logs
                ...
                .iter()
                .enumerate()
                .filter_map(|(i, p)| if re.is_match(&p.raw) { Some(i) } else { None })
                .collect()
        } else {
        ...
        egui::ScrollArea::vertical()
            ...
            .show_rows(
                ...
                row_h,
                if self.logs.follow {
                    rows + self.logs.follow_pad_rows
                } else {
                    rows
                },
                |ui, range| {
    ...
    pub(crate) fn ui_details(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            if let Some(uid) = self.details.selected {
                ...
                if is_detached {
                    if ui
                        .small_button("Reattach")
                        .on_hover_text("Close window and re-open as a tab")
                        .clicked()
                    {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .selectable_label(matches!(tab, DetailsPaneTab::Edit), "Edit")
                .clicked()
            {
                self.details.active_tab = DetailsPaneTab::Edit;
            }
            if ui
                .selectable_label(matches!(tab, DetailsPaneTab::Logs), "Logs")
                .clicked()
            {
            ...
            if ui
                ...
                .clicked()
            {
                self.details.active_tab = DetailsPaneTab::Describe;
            }
            if ui
                .selectable_label(matches!(tab, DetailsPaneTab::Graph), "Graph")
                .on_hover_text("List-based relationships")
                .clicked()
            {
                self.details.active_tab = DetailsPaneTab::Graph;
            }
        });
        ui.separator();
        egui::ScrollArea::vertical()
                    ...
                    DetailsPaneTab::Describe => {
                        ui.horizontal(|ui| {
                            if ui.small_button("Refresh").clicked() {
                                if let Some(uid) = self.details.selected {
                                    self.start_describe_task(uid);
                                }
                            }
                            if self.describe.running {
                                ui.add(egui::Spinner::new());
                            }
                            if let Some(err) = &self.describe.error {
                        ...
                        if self.details.selected.is_some() {
                            let need_fetch = match (self.describe.uid, self.details.selected) {
                                (Some(u0), Some(u1)) => u0 != u1,
                                _ => true,
                            };
                            if need_fetch && !self.describe.running {
                                if let Some(uid) = self.details.selected {
                                    self.start_describe_task(uid);
                                }
                            }
                        }
                        let mut text = if self.describe.text.is_empty() {
                            String::from("(no output yet)")
                        } else {
                            self.describe.text.clone()
                        };
    ...
    fn ui_graph(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(sel) = self.details.selected {
            let need_fetch = match (self.graph.uid, self.details.selected) {
                (Some(u0), Some(u1)) => u0 != u1,
                _ => true,
            };
        ...
        ui.horizontal(|ui| {
            ...
            if let Some(err) = &self.graph.error {
                ui.colored_label(ui.visuals().error_fg_color, err);
            }
            ui.separator();
            ui.label("Mode:");
        ...
        match self.graph.mode {
            GraphViewMode::Classic => {
                let mut text = if self.graph.text.is_empty() {
                    String::from("(no graph yet)")
                } else {
                    self.graph.text.clone()
                };
                let te = egui::TextEdit::multiline(&mut text)
                    .font(egui::TextStyle::Monospace)
                    .desired_rows(24)
    ...
    fn ui_graph_atlas(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .small_button("Fit")
                .on_hover_text("Fit to root and neighbors")
                .clicked()
            {
                self.graph.details_fit_for = None;
            }
        ...
        for node in &model.nodes {
            match &node.role {
                GraphNodeRole::Root => {
                    root_id = Some((node.id.clone(), node.label.clone()));
                }
                GraphNodeRole::OwnerChain(depth) => {
                    owner_chain.push((node.id.clone(), *depth, node.label.clone()));
                }
        ...
        if let Some((rid, _)) = &root_id {
            positions.insert(rid.clone(), egui::pos2(0.0, 0.0));
        }
        for (id, depth, _label) in &owner_chain {
            let d = *depth as f32;
            positions.insert(id.clone(), egui::pos2(0.0, -d * 120.0));
        }
        ...
        if let Some(uid) = self.graph.uid {
            if self.graph.details_fit_for != Some(uid) {
                ...
                if let Some((rid, _)) = &root_id {
                    if let Some(p) = positions.get(rid) {
                        consider.push(*p);
                    }
                }
                for (_id, _d, _l) in &owner_chain {
                    if let Some(p) = positions.get(_id.as_str()) {
                        consider.push(*p);
                    }
                }
        ...
        for (kind, items) in groups {
            let base = group_pos
                .get(&kind)
                .copied()
                .unwrap_or(egui::pos2(180.0, 0.0));
            let p = to_screen(base);
            if let Some((rid, _)) = &root_id {
                if let Some(rp) = positions.get(rid) {
                    painter.line_segment(
                        [to_screen(*rp), p],
            ...
            painter.circle_stroke(
                p,
                18.0 * z.clamp(0.5, 1.3),
                egui::Stroke::new(2.0, ui.visuals().widgets.noninteractive.bg_stroke.color),
            );
            let text = format!("{} ({})", kind, items.len());
            painter.text(
                p + egui::vec2(0.0, 24.0),
            ...
            if resp.clicked() {
                if self.graph.details_expanded_kinds.contains(&kind) {
                    self.graph.details_expanded_kinds.remove(&kind);
                } else {
                    self.graph.details_expanded_kinds.insert(kind.clone());
                }
            }
            if self.graph.details_expanded_kinds.contains(&kind) {
    ...
    fn ui_exec(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if self.logs.containers.is_empty() {
                ...
                if selected != current {
                    self.exec.container = Some(selected);
                    if self.exec.running && changed_container {
                        self.stop_exec_task();
                    }
                }
            }
        ...
        if self.exec.mode_oneshot {
            ...
            let display = if shown == 0 {
                ...
                display.lines().rev().collect::<Vec<_>>().join("\n")
            };
            ...
            if self.exec.term.is_none() {
                self.exec.term = Some(crate::ui::term::UiTerminal::new());
            }
        ...
        if !self.exec.mode_oneshot {
            ui.horizontal(|ui| {
                ...
                let edit = ui.add_enabled(
                    self.exec.running,
                    egui::TextEdit::singleline(&mut self.exec.stdin_buf).desired_width(400.0),
                );
                let want_send = send_btn.clicked()
                    || (self.exec.running
                        && edit.lost_focus()
        ...
        if self.exec.dropped > 0 {
            ui.colored_label(
                ui.visuals().warn_fg_color,
                format!("dropped: {}", self.exec.dropped),
            );
        }
        if sent_resize {
            ui.colored_label(ui.visuals().weak_text_color(), "resized");
        }
        let hint = if self.exec.mode_oneshot {
            if self.exec.running {
                "running…"
            } else {
                "enter a command and click Run"
            }
        } else if self.exec.running {
            if self.exec.focused {
                "typing active • Esc to release"
            } else {
                "click inside to type"
            }
        } else {
            "press Start to open a shell"
        };
        ...
        ui.horizontal(|ui| {
            ...
            if ui.button("Browse…").clicked() {
                if let Some(path) = rfd::FileDialog::new()
                    .set_title("Choose terminal app or binary")
                    .pick_file()
                {
            ...
            if ui
                ...
                .on_hover_text("Launch configured terminal with kubectl exec -it")
                .clicked()
            {
                self.open_external_exec();
            }
        });
        ...
        if self.exec.focused && self.exec.running {
            ...
            ui.input(|i| {
                for ev in &i.events {
                    match ev {
                        egui::Event::Text(s) => {
                            to_send.extend_from_slice(s.as_bytes());
                        }
                        egui::Event::Key {
                            key,
                            pressed: true,
            ...
            if !to_send.is_empty() {
                if let Some(tx) = self.exec.input.clone() {
                    let _ = tx.try_send(to_send);
                }
            }
        }
    }
    ...
    fn ui_service_logs(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            if ui
                .add(egui::DragValue::new(&mut vis).range(100..=self.svc_logs.ring_cap as i32))
                .on_hover_text("Max visible lines when following")
                .changed()
            {
            ...
            if theme != self.svc_logs.prefix_theme {
                self.svc_logs.prefix_theme = theme;
        ...
        ui.horizontal(|ui| {
            ...
            if resp.changed() {
                ...
                if text.is_empty() {
                    ...
                    self.svc_logs.grep_error = None;
                } else if let Ok(re) = regex::Regex::new(&text) {
                    self.svc_logs.grep_cache = Some((text, re));
                    self.svc_logs.grep_error = None;
                } else {
                    self.svc_logs.grep_cache = None;
            ...
            if self.svc_logs.dropped > 0 {
                ui.separator();
                ui.colored_label(
                    ui.visuals().error_fg_color,
                    format!("dropped: {}", self.svc_logs.dropped),
                );
            }
        ...
        let mut indices: Vec<usize> = if let Some((_t, re)) = &self.svc_logs.grep_cache {
            ...
            (0..self.svc_logs.ring.len()).collect()
        };
        ...
        if self.svc_logs.follow {
            let len = indices.len();
            let take = self.svc_logs.visible_follow_limit.min(len);
            indices = indices.split_off(len - take);
        }
        ...
        egui::ScrollArea::vertical()
            ...
            .show_rows(
                ...
                row_h,
                if self.svc_logs.follow {
                    rows + self.svc_logs.follow_pad_rows
                } else {
                    rows
                },
                |ui, range| {
                    for local_row in range.clone() {
                        if self.svc_logs.follow && local_row >= rows {
                            ui.add_space(row_h);
                            continue;
                        }
                        if let Some(&ring_idx) = indices.get(local_row) {
    ...
    pub(crate) fn select_row(&mut self, it: LiteObj) {
        ...
        let prefetch_ms: u64 = std::env::var("ORKA_DETAILS_PREFETCH_MS")
            .ok()
        ...
        self.details.task = Some(tokio::spawn(async move {
            ...
            let fetch = async {
                ...
                match api.get_raw(reference).await {
                    Ok(bytes) => {
                        let maxb: usize = std::env::var("ORKA_DETAILS_MAX_BYTES")
                            .ok()
                        ...
                        if let Some(tx) = tx_opt.as_ref() {
                            ...
                            if let Some(entries) = secret_entries {
                                let _ = tx.send(UiUpdate::SecretReady { uid, entries });
                            }
                            if let Some(v) = containers {
                                let _ = tx.send(UiUpdate::PodContainers(v));
                            }
                            if let Some(pp) = pod_ports {
                                let _ = tx.send(UiUpdate::PodPorts(pp));
                            }
                        }
                    }
                    Err(e) => {
