...
use orka_api::{api_ops, OpsLogOptions as LogOptions};
use tracing::info;
...
impl OrkaGuiApp {
    ...
    pub(crate) fn stop_logs_task(&mut self) {
        ...
        if let Some(task) = self.logs.task.take() {
            task.abort();
        }
    ...
    pub(crate) fn start_logs_task(&mut self) {
        ...
        let Some((ns, pod)) = self.current_pod_selection() else {
            self.last_error = Some("logs: select a Pod first".into());
            return;
        };
                ...
                let client = match orka_kubehub::get_kube_client().await {
                    ...
                    Err(e) => {
                        if let Some(tx) = &tx_opt {
                            let _ = tx.send(UiUpdate::LogError(e.to_string()));
                        }
                        return;
                    }
                };
                ...
                match pods.get(&pod).await {
                    Ok(p) => {
                        if let Some(spec) = p.spec {
                            ...
                            if let Some(cs) = spec.init_containers {
                                for c in cs {
                                    names.push(c.name);
                                }
                            }
                            if let Some(cs) = spec.ephemeral_containers {
                                for c in cs {
                                    names.push(c.name);
                                }
                            }
                    ...
                    Err(e) => {
                        if let Some(tx) = &tx_opt {
                            let _ = tx.send(UiUpdate::LogError(format!("logs: pod spec: {}", e)));
                        }
                    }
                }
                if names.is_empty() {
                    ...
                    match ops.logs(Some(&ns), &pod, None, opts).await {
                        Ok(handle) => {
                            if let Some(tx) = &tx_opt {
                                let _ = tx.send(UiUpdate::LogStarted(handle.cancel));
                            }
                            let mut rx = handle.rx;
                            while let Some(line) = rx.recv().await {
                                if let Some(tx) = &tx_opt {
                                    let _ = tx.send(UiUpdate::LogLine(line));
                                }
                            }
                            if let Some(tx) = &tx_opt {
                                let _ = tx.send(UiUpdate::LogEnded);
                            }
                        }
                        Err(e) => {
                            if let Some(tx) = &tx_opt {
                    ...
                    struct CancelGuard {
                        inner: Option<orka_api::CancelHandle>,
                    }
                        ...
                        tasks.push(tokio::spawn(async move {
                            match ops2.logs(Some(&ns2), &pod2, Some(&cname), opts2).await {
                                Ok(mut h) => {
                                    ...
                                    let prefix = match color_code_for(&cname, theme2) {
                                        Some(code) => format!("\x1b[{}m[{}]\x1b[0m ", code, cname),
                                        None => format!("[{}] ", cname),
                                    };
                                    while let Some(line) = h.rx.recv().await {
                                        if let Some(tx) = &tx2 {
                                            let _ = tx.send(UiUpdate::LogLine(format!(
                                                "{}{}",
                                                prefix, line
                                            )));
                                        }
                                ...
                                Err(e) => {
                                    if let Some(tx) = &tx2 {
                                        let _ = tx.send(UiUpdate::LogError(format!(
                                            "logs[{}]: {}",
                                            cname, e
                                        )));
                                    }
