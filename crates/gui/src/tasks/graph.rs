...
impl OrkaGuiApp {
    ...
    pub(crate) fn start_graph_task(&mut self, uid: Uid) {
        ...
        if let Some(task) = self.graph.task.take() { task.abort(); }
        if let Some(stop) = self.graph.stop.take() { let _ = stop.send(()); }
        ...
        let gvk = match gvk_opt { Some(k) => k, None => { self.graph.running = false; return; } };
        let name = match name_opt { Some(n) => n, None => { self.graph.running = false; return; } };
...
fn owner_from_value(v: &serde_json::Value) -> Option<(String, String, String, String)> {
    ...
    let owners = v.get("metadata").and_then(|m| m.get("ownerReferences")).and_then(|x| x.as_array())?;
    // Prefer controller=true
    let mut cand = None;
    for o in owners {
        let kind = o.get("kind").and_then(|x| x.as_str()).unwrap_or("").to_string();
        let name = o.get("name").and_then(|x| x.as_str()).unwrap_or("").to_string();
        let apiv = o.get("apiVersion").and_then(|x| x.as_str()).unwrap_or("");
        let (group, version) = if let Some((g, v)) = apiv.split_once('/') { (g.to_string(), v.to_string()) } else { (String::new(), apiv.to_string()) };
        let is_controller = o.get("controller").and_then(|x| x.as_bool()).unwrap_or(false);
        if is_controller { return Some((group, version, kind, name)); }
        if cand.is_none() { cand = Some((group, version, kind, name)); }
    }
    cand
}
...
async fn build_graph_text(
    ...
    let header = if gvk.namespaced { format!("{} {}/{}", gvk.kind, ns_str, name) } else { format!("{} {}", gvk.kind, name) };
    ...
    while let Some((group, version, kind, oname)) = owner_from_value(&cur_v) {
        steps += 1;
        let _ = std::fmt::write(&mut out, format_args!("  -> {}/{} {}\n", if group.is_empty() { version.clone() } else { format!("{}/{}", group, version) }, kind, oname));
        if steps >= 5 { break; }
    ...
    match kind {
        "Pod" => {
            ...
            if !cms.is_empty() { let _ = std::fmt::write(&mut out, format_args!("  ConfigMaps: {}\n", cms.join(", "))); }
            if !secs.is_empty() { let _ = std::fmt::write(&mut out, format_args!("  Secrets: {}\n", secs.join(", "))); }
        }
        "Service" => {
            ...
            if let Some(sel) = v.get("spec").and_then(|s| s.get("selector")).and_then(|x| x.as_object()) {
                ...
                let _ = std::fmt::write(&mut out, format_args!("  Pods: {} (selector: {})\n", count, if selector.is_empty() { "(none)".to_string() } else { selector }));
            }
        }
        _ => {}
    }
...
async fn count_pods_for_selector(ns: Option<&str>, selector: &str) -> Result<usize, String> {
    ...
    let client = orka_kubehub::get_kube_client().await.map_err(|e| e.to_string())?;
    let (ar, namespaced) = orka_kubehub::get_api_resource("v1/Pod").await.map_err(|e| e.to_string())?;
    ...
    if !selector.is_empty() { lp = lp.labels(selector); }
    lp = lp.limit(500);
