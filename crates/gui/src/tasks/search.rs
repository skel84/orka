...
use crate::{OrkaGuiApp, UiUpdate, SearchExplain};
...
impl OrkaGuiApp {
    pub(crate) fn start_search_task(&mut self) {
        ...
        let ns_opt = if k.namespaced && !self.selection.namespace.is_empty() { Some(self.selection.namespace.clone()) } else { None };
        if self.search.query.trim().is_empty() { self.search.hits.clear(); self.search.explain = None; self.search.partial = false; return; }
        ...
        let tx = if let Some(tx0) = &self.watch.updates_tx { tx0.clone() } else {
    ...
    pub(crate) fn rebuild_search_preview(&mut self) {
        ...
        if raw.is_empty() || self.results.rows.is_empty() { return; }
        ...
        for it in &self.results.rows {
            ...
            if free_q.is_empty() || score >= 0f32 { scored.push((it.uid, score)); }
        }
        scored.sort_by(|a, b| b.1.total_cmp(&a.1).then_with(|| {
            let an = self.results.index.get(&a.0).and_then(|i| self.results.rows.get(*i)).map(|o| o.name.clone()).unwrap_or_default();
            let bn = self.results.index.get(&b.0).and_then(|i| self.results.rows.get(*i)).map(|o| o.name.clone()).unwrap_or_default();
            an.cmp(&bn)
        }));
        self.search.preview = scored.into_iter().take(10).collect();
        self.search.preview_sel = if self.search.preview.is_empty() { None } else { Some(0) };
    }
}
