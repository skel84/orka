                ...
                Ok(s) => {
                    ...
                    if let Some(addr) = metrics_addr {
                        if let Ok((ib, id)) = scrape_index_metrics(&addr).await {
                            let _ = tx.send(UiUpdate::MetricsReady { index_bytes: ib, index_docs: id });
                        }
                    }
                }
                Err(e) => { let _ = tx.send(UiUpdate::Error(format!("stats: {}", e))); }
            }
        }));
    }
}
...
async fn scrape_index_metrics(addr: &str) -> Result<(Option<u64>, Option<u64>), ()> {
    ...
    for line in body.lines() {
        ...
        if line.starts_with("index_bytes") {
            if let Some(val) = line.split_whitespace().last() { if let Ok(v) = val.parse::<f64>() { index_bytes = Some(v as u64); } }
        } else if line.starts_with("index_docs") {
            if let Some(val) = line.split_whitespace().last() { if let Ok(v) = val.parse::<f64>() { index_docs = Some(v as u64); } }
        }
    }
