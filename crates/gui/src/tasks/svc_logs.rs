...
impl OrkaGuiApp {
    pub(crate) fn current_service_selection(&self) -> Option<(String, String)> {
        ...
        if !(kind.group.is_empty() && kind.version == "v1" && kind.kind == "Service") { return None; }
    ...
    pub(crate) fn stop_service_logs_task(&mut self) {
        ...
        if let Some(task) = self.svc_logs.task.take() { task.abort(); }
    ...
    pub(crate) fn start_service_logs_task(&mut self) {
        ...
        let Some((ns, svc)) = self.current_service_selection() else { self.last_error = Some("service logs: select a Service first".into()); return; };
        ...
        let task = tokio::spawn(async move {
            ...
            let selector = match selector {
                Some(serde_json::Value::Object(map)) if !map.is_empty() => {
                    ...
                    for (k, vv) in map.into_iter() { if let Some(val) = vv.as_str() { parts.push(format!("{}={}", k, val)); } }
                    parts.join(",")
                }
                _ => String::new(),
            };
            // List pods by selector using kube
            let client = match orka_kubehub::get_kube_client().await { Ok(c) => c, Err(e) => { if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogError(format!("svc_logs: kube client: {}", e))); } return; } };
            use kube::api::{Api, ListParams};
            use k8s_openapi::api::core::v1::Pod;
            let pods_api: Api<Pod> = Api::namespaced(client, &ns);
            let lp = if selector.is_empty() { ListParams::default() } else { ListParams::default().labels(&selector) };
            let mut pod_names: Vec<String> = Vec::new();
            match pods_api.list(&lp).await {
                Ok(list) => { for p in list.items { if let Some(name) = p.metadata.name { pod_names.push(name); } } }
                Err(e) => { if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogError(format!("svc_logs: list pods: {}", e))); } }
            }
            if pod_names.is_empty() {
                if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogError("svc_logs: no matching pods".into())); }
                return;
            }
            // Start one log stream per pod and forward lines with a [pod] prefix
            let ops = api_ops(api.as_ref());
            let mut handles: Vec<(tokio::task::JoinHandle<()>, orka_api::CancelHandle)> = Vec::new();
            if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogStarted); }
            ...
            for pod in pod_names.into_iter() {
                ...
                let pod2 = pod.clone();
                let opts = orka_api::OpsLogOptions { follow, tail_lines: tail, since_seconds: since };
                match ops.logs(Some(&ns), &pod, None, opts).await {
                    Ok(mut h) => {
                        let theme2 = theme;
                        let t = tokio::spawn(async move {
                            let prefix = match color_code_for(&pod2, theme2) { Some(code) => format!("\x1b[{}m[{}]\x1b[0m ", code, pod2), None => format!("[{}] ", pod2) };
                            while let Some(line) = h.rx.recv().await { if let Some(tx) = &tx2 { let _ = tx.send(UiUpdate::SvcLogLine(format!("{}{}", prefix, line))); } }
                        });
                        handles.push((t, h.cancel));
                    }
                    Err(e) => { if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogError(format!("svc_logs: {}: {}", pod2, e))); } }
                }
            }
            // Wait for all to end gracefully; on task abort Drop will cancel
            for (t, _c) in handles.into_iter() { let _ = t.await; }
            if let Some(tx) = &tx_opt { let _ = tx.send(UiUpdate::SvcLogEnded); }
