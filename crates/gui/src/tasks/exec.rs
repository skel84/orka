...
impl OrkaGuiApp {
    pub(crate) fn stop_exec_task(&mut self) {
        if let Some(cancel) = self.exec.cancel.take() {
            cancel.cancel();
        }
        if let Some(task) = self.exec.task.take() {
            task.abort();
        }
    ...
    pub(crate) fn start_exec_task(&mut self) {
        ...
        let Some((ns, pod)) = self.current_pod_selection() else {
            self.last_error = Some("exec: select a Pod first".into());
            return;
        };
        let container = self.exec.container.clone();
        let pty = self.exec.pty;
        let cmd_text = if self.exec.cmd.trim().is_empty() {
            "/bin/sh".to_string()
        } else {
            self.exec.cmd.clone()
        };
        ...
        if cmd.is_empty() {
            self.last_error = Some("exec: command is empty".into());
            return;
        }
        ...
        let task = tokio::spawn(async move {
            ...
            match ops
                .exec_stream(Some(&ns), &pod, container.as_deref(), &cmd, pty)
                .await
            {
                Ok(mut h) => {
                    // Notify UI with handle pieces
                    if let Some(tx) = &tx_opt {
                        let _ = tx.send(UiUpdate::ExecStarted {
                            cancel: h.cancel,
                            input: h.input.clone(),
                            resize: h.resize.clone(),
                        });
                    }
                    // Bridge output
                    while let Some(chunk) = h.rx.recv().await {
                        ...
                        if let Some(tx) = &tx_opt {
                            let _ = tx.send(UiUpdate::ExecData(s));
                        }
                    }
                    if let Some(tx) = &tx_opt {
                        let _ = tx.send(UiUpdate::ExecEnded);
                    }
                }
                Err(e) => {
                    if let Some(tx) = &tx_opt {
                        let _ = tx.send(UiUpdate::ExecError(e.to_string()));
                    }
                }
            }
        });
    ...
    pub(crate) fn open_external_exec(&mut self) {
        ...
        if let Some(ctx) = &self.current_context {
            args.extend(["--context".to_string(), ctx.clone()]);
        }
        args.extend([
            "-n".to_string(),
            ns.clone(),
        ...
        // Default command
        let cmd = if self.exec.cmd.trim().is_empty() {
            "/bin/sh".to_string()
        } else {
            self.exec.cmd.clone()
        };
        ...
        let launched = {
            ...
            let ghostty_like = chosen.eq_ignore_ascii_case("ghostty")
                || chosen.to_ascii_lowercase().contains("ghostty");
    ...
    pub(crate) fn start_exec_oneshot_task(&mut self) {
        ...
        let container = self.exec.container.clone();
        let cmd_text = if self.exec.cmd.trim().is_empty() {
            "/bin/sh -c 'echo shell missing'".to_string()
        } else {
            self.exec.cmd.clone()
        };
        ...
        let task = tokio::spawn(async move {
            ...
            let mut child = match base.spawn() {
                ...
                Err(e) => {
                    if let Some(tx) = &tx_opt {
                        let _ = tx.send(UiUpdate::ExecError(format!("exec: spawn: {}", e)));
                    }
                    return;
                }
            };
...
fn args_to_cmd(bin: &str, args: &[String]) -> String {
    ...
    for a in args {
        s.push(' ');
        s.push_str(&shell_escape::escape(a.as_str().into()));
    }
    s
}
