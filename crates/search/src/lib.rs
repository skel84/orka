...
impl Index {
    ...
    pub fn build_from_snapshot_with_meta(
        ...
        if let Some(pairs) = fields {
            for (k, v) in pairs.iter() {
                field_ids.insert(k.clone(), *v);
            }
        }
        let postings_cap: Option<usize> = std::env::var("ORKA_MAX_POSTINGS_PER_KEY")
            .ok()
        ...
        for (i, o) in snap.items.iter().enumerate() {
            ...
            for (k, v) in o.labels.iter() {
                ...
                if let Some(cap) = postings_cap {
                    if vec.len() >= cap {
                        truncated_keys_total += 1;
                    } else {
                        vec.push(li);
                    }
                } else {
        ...
        if let Some(cap) = std::env::var("ORKA_MAX_INDEX_BYTES")
            .ok()
            .and_then(|s| s.parse::<usize>().ok())
        {
            if approx_bytes > cap {
                ...
                let (after, events) =
                    enforce_index_cap_flat(&mut flat, cap, approx_bytes, &g_names);
                approx_bytes = after;
                for e in events {
    ...
    pub fn build_from_snapshot_with_meta_planner(
        ...
        let mut shards: Vec<IndexShard> = (0..shards_n)
            .map(|_| IndexShard {
                texts: Vec::new(),
                namespaces: Vec::new(),
                names: Vec::new(),
        ...
        for (i, o) in snap.items.iter().enumerate() {
            ...
            shard
                .projected
                .push(o.projected.clone().into_iter().collect());
        ...
        for sh in shards.iter() {
            ...
            approx_bytes += sh
                .projected
                .iter()
                .map(|v| v.iter().map(|(_id, s)| s.len()).sum::<usize>())
                .sum::<usize>();
            approx_bytes += sh
                .label_post
                .values()
        ...
        {
            if approx_bytes > cap {
                warn!(approx_bytes, cap, "index_bytes exceeds ORKA_MAX_INDEX_BYTES; consider increasing shards or capping postings");
            }
        }
        metrics::gauge!("index_postings_truncated_keys", truncated_keys_total as f64);
    ...
    pub fn search_with_debug_opts(
        &self,
        q: &str,
        limit: usize,
        opts: SearchOpts,
    ) -> (Vec<Hit>, SearchDebugInfo) {
        ...
        for tok in q.split_whitespace() {
            if let Some(rest) = tok.strip_prefix("ns:") {
                ns_filter = Some(rest);
                continue;
            }
            if let Some(rest) = tok.strip_prefix("k:") {
                if !rest.is_empty() {
                    kind_filters.push(rest.to_string());
                    continue;
                }
            }
            if let Some(rest) = tok.strip_prefix("g:") {
                if !rest.is_empty() {
                    group_filters.push(rest.to_string());
                    continue;
                }
            }
            ...
            if let Some(rest) = tok.strip_prefix("label:") {
                if rest.contains('=') {
                    label_filters.push(rest.to_string());
                    continue;
                }
                if !rest.is_empty() {
                    label_key_filters.push(rest.to_string());
                    continue;
                }
            }
            if let Some(rest) = tok.strip_prefix("anno:") {
                if rest.contains('=') {
                    anno_filters.push(rest.to_string());
                    continue;
                }
                if !rest.is_empty() {
                    anno_key_filters.push(rest.to_string());
                    continue;
                }
            }
            free_terms.push(tok);
        ...
        let mut candidates: Vec<usize> = if let Some(ns) = ns_filter {
            (0..sh.texts.len())
                .filter(|i| sh.namespaces.get(*i).map(|s| s == ns).unwrap_or(false))
                .collect()
        } else {
        ...
        if let Some(maxc) = opts.max_candidates {
            if candidates.len() > maxc {
                candidates.truncate(maxc);
            }
        }
        ...
        'doc: for li in candidates.into_iter() {
            for (pid, ref val) in field_filters.iter() {
                let ok = sh
                    .projected
                    .get(li)
                    .map(|vec| vec.iter().any(|(id, v)| id == pid && v == val))
                    .unwrap_or(false);
                if !ok {
                    continue 'doc;
                }
        ...
        hits.truncate(limit);
        let dbg = SearchDebugInfo {
            total,
            after_ns: after_ns_sum,
            after_label_keys: after_label_keys_sum,
            after_labels: after_labels_sum,
            after_anno_keys: after_anno_keys_sum,
            after_annos: after_annos_sum,
            after_fields: passed_fields_total,
        };
...
fn enforce_index_cap_flat(
    flat: &mut FlatIndex,
    cap: usize,
    approx_before: usize,
    g_names: &[String],
) -> (usize, Vec<PressureEvent>) {
    ...
    if approx > cap {
        ...
        if trimmed > 0 || dropped > 0 {
            ...
            events.push(PressureEvent {
                phase: "value_postings",
                trimmed_bytes: approx.saturating_sub(after),
...
fn prune_value_postings_flat(flat: &mut FlatIndex) -> (usize, usize) {
    ...
    let mut keys = 0usize;
    for (_k, v) in flat.label_post.drain() {
        bytes += v.len() * slot;
        keys += 1;
    }
    for (_k, v) in flat.anno_post.drain() {
        bytes += v.len() * slot;
        keys += 1;
    }
    (bytes, keys)
...
fn prune_key_postings_flat(flat: &mut FlatIndex) -> (usize, usize) {
    ...
    let mut bytes = 0usize;
    let mut keys = 0usize;
    for (_k, v) in flat.label_key_post.drain() {
        bytes += v.len() * slot;
        keys += 1;
    }
    for (_k, v) in flat.anno_key_post.drain() {
        bytes += v.len() * slot;
        keys += 1;
    }
    (bytes, keys)
...
mod tests {
    ...
    fn label_and_anno_filters() {
        ...
        assert_eq!(
            hits2.len(),
            2,
            "label key existence should match both items"
        );
    ...
    fn field_filter_matches_projected() {
        ...
        let pairs = vec![
            ("spec.foo".to_string(), 1u32),
            ("spec.bar".to_string(), 2u32),
        ];
        let idx =
            Index::build_from_snapshot_with_meta(&s, Some(&pairs), Some("ConfigMap"), Some(""));
...
mod opts_tests {
    ...
    fn max_candidates_caps_evaluation() {
        ...
        let idx = Index::build_from_snapshot(&s);
        let (hits, _dbg) = idx.search_with_debug_opts(
            "ns:ns",
            10,
            SearchOpts {
